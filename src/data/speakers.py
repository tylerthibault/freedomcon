speakers = [
	{
		'name': 'Josh McPherson',
		'image': 'img/speakers/JoshM_2.png',
		'alt': 'Josh McPherson',
		'titles': ['Lead Pastor, Grace City Church', 'Founder, Stronger Man Nation'],
		'bio': """
			Josh McPherson is the founding and lead pastor of Grace City Church in Wenatchee, Washington, where he and his wife Sharon planted with a small team in 2008. Since then it has grown into a strong, life-giving church family known for its grit, generosity, and bold gospel culture. Before stepping into full-time ministry, Josh worked in real estate, construction, and law enforcement, giving him blue-collar credibility with the men he leads. His family lives in a home he built in the middle of an orchard they farm. Josh is the author of multiple books. He founded Stronger Man Nation, a growing movement that helps dads raise boys to be men, and helps men get stronger in every area of life. Strong men, families, churches, and communities is his passion. He is the visionary founder of Garden City Academy, the first K-12 Christian school in the state to start wtih full-grades, sports, and drama from scratch. He also founded Vector College, currently an accredited 2-year liberty arts college raising men and women to lead Christiandom to save the West. Known for his straightforward, unapologetic preaching and visionary leadership, Josh is committed to drawing greatness out of men for Jesus’ sake, while building teams, churches, and institutions that strengthen the family and advance the Kingdom of God in the heart of Washington State.
		 """,
		'shrink': 0.85,
		"orgs": [{"icon": "church", "name": "Grace City Church", "subtitle": "Wenatchee, WA"}, {"icon": "shield", "name": "Stronger Man Nation", "subtitle": "Equipping men. Building leaders."}, {"icon": "cap", "name": "Garden City Academy", "subtitle": "Training the next generation."}, {"icon": "cap", "name": "Vector College", "subtitle": "Training the next generation."}],
},
	{
		'name': 'Eric Metaxas',
		'image': 'img/speakers/EricM_3.png',
		'alt': 'Eric Metaxas',
		'titles': ['#1 NY Times best-selling author', 'Founder, Socrates in the City', 'Host, The Eric Metaxas Show'],
		'bio': """
			Eric Metaxas is a #1 New York Times bestselling author, cultural commentator, and unapologetic defender of Christian faith and Western freedom. He’s known worldwide for his powerful biographies of men who stood up to tyranny—William Wilberforce, Dietrich Bonhoeffer, and Martin Luther—each one a call to moral courage in the face of political and spiritual darkness. Eric hosts a nationally syndicated radio show and speaks across the country, challenging believers to resist soft totalitarianism, fight for religious liberty, and recover America’s forgotten promise of ordered liberty under God. With books like “If You Can Keep It” and “Miracles,” he has emerged as a leading voice urging men to think deeply, live boldly, and push back against the cultural revolution with truth and joy.
		 """,
		"orgs": [{"icon": "book", "name": "Socrates in the City", "subtitle": "Intellectual faith conversations"}, {"icon": "mic", "name": "The Eric Metaxas Show", "subtitle": "Nationally syndicated radio"}, {"icon": "star", "name": "NY Times Bestselling", "subtitle": "Bonhoeffer & Wilberforce"}],
        'shrink': 0.87,
	},
	{
		'name': 'John Lovell',
		'image': 'img/speakers/JohnL.png',
		'alt': 'John Lovell',
		'titles': ['Lead Pastor, Grace City, GA', 'Founder, Warrior Poet Society'],
		'bio': """
			John Lovell is a former Army Ranger, combat veteran, and the founder of Warrior Poet Society—a nationwide brotherhood teaching men to be dangerous, disciplined, and deeply grounded in truth. He served in the elite 2nd Ranger Battalion with multiple deployments before leaving special operations to become a Christian missionary in Central America, where his passion for the gospel and courage under fire were forged together. Today, John is a best-selling author, firearms trainer, and media leader whose content has garnered over 100 million views, calling men to embrace the paradox of being both lion and lamb, protector and shepherd. Through Warrior Poet Society’s training, gear, and network, he is equipping police, military, and everyday husbands and fathers to live free, reject cowardice, and stand guard over their homes and communities.
		 """,
	        "orgs": [{"icon": "shield", "name": "Warrior Poet Society", "subtitle": "Dangerous. Disciplined. Grounded."}, {"icon": "star", "name": "Grace City Rome", "subtitle": "Lead Pastor"}, {"icon": "star", "name": "Author", "subtitle": "Best-selling autho, Book1"}],
            'shrink': 0.95,
},
	{
		'name': 'Russell Johnson',
		'image': 'img/speakers/RussellJ_3.png',
		'alt': 'Russell Johnson',
		'titles': ['Lead Pastor, The Pursuit Church'],
		'bio': """
			Russell Johnson is the founding pastor of The Pursuit, a Spirit-filled church in the Pacific Northwest that has become a rally point for revival, repentance, and courage in one of the most progressive corners of America. Born and raised in Seattle, Russell spent the early part of his adult life in politics before God called him out of the back rooms and into the pulpit, where he now leads a growing multi-campus church family. Since launching The Pursuit in 2014 with his wife Marija, he has preached an unfiltered message of holiness, spiritual power, and resistance to godless ideology, calling believers to stand their ground rather than retreat from cultural pressure. Under his leadership, The Pursuit has grown from a small gathering in a drafty barn into a force for spiritual awakening in the region, where men are challenged to encounter God and live like it actually matters.
		 """,
		'shrink': 0.92,
        'image_x': -10,
	        "orgs": [{"icon": "church", "name": "The Pursuit Church", "subtitle": "Pacific Northwest revival"}],
},
	{
		'name': 'Graham Allen',
		'image': 'img/speakers/GrahamA_3.png',
		'alt': 'Graham Allen',
		'titles': ['U.S. Army Veteran', 'The Graham Allen Show & Dear America'],
		'bio': """
			Graham Allen is a 12-year Army combat veteran turned conservative firebrand whose straight-talk commentary has made him one of the loudest pro-America voices online. Best known as “The Daily Rants guy,” he first exploded onto the scene with unfiltered videos defending the flag, law enforcement, and traditional values against the madness of cancel culture. Graham is the host of The Graham Allen Show (formerly the Dear America podcast), where he tackles politics, faith, and culture with the kind of blunt honesty you’d hear in a barracks or a mechanic’s shop, not a newsroom. A combat vet, husband, and father, he calls men to stop whining, take responsibility, and fight for their families and their country with courage that matches their convictions.
		 """,
	        "orgs": [{"icon": "mic", "name": "The Graham Allen Show", "subtitle": "Conservative commentary"}, {"icon": "star", "name": "U.S. Army", "subtitle": "12-year combat veteran"}, {"icon": "star", "name": "Author", "subtitle": "Best-selling author"}],
},
	{
		'name': 'Mark Driscoll',
		'image': 'img/speakers/Mark_4.png',
		'alt': 'Mark Driscoll',
		'titles': ['Lead Pastor, Trinity Church', 'Founder, Real Faith'],
		'bio': """
			Mark Driscoll is a battle-tested pastor, church planter, and author who has spent three decades calling men to spiritual grit and courage. Today he serves as the founding pastor of Trinity Church in Scottsdale, Arizona, preaching a direct, no-apologies message of repentance, masculinity, and allegiance to Christ. A prolific author and outspoken cultural critic, he’s known for confronting passive, consumer Christianity and calling men to lead their families, serve their churches, and stand firm in a collapsing culture.
		 """,
	        "orgs": [{"icon": "church", "name": "Trinity Church", "subtitle": "Scottsdale, AZ"}, {"icon": "book", "name": "Real Faith", "subtitle": "Lead Pastor"}],
},
	{
		'name': 'Chad Robichaux',
		'image': 'img/speakers/ChadR-Sitting.png',
		'alt': 'Chad Robichaux',
		'titles': ['Force Recon Marine', 'DoD Contractor', 'Founder, Mighty Oaks Foundation', 'Host, The Resilient Show', 'Best Selling Author & Speaker', 'Pro MMA Champion'],
		'bio': """
			Chad Robichaux is a former Force Recon Marine, JSOC operator, and pro MMA champion who walked through the darkest valleys of war, PTSD, and near suicide—and chose to fight his way back. He served eight deployments to Afghanistan with a Joint Special Operations Command task force, then channeled that experience into founding the Mighty Oaks Foundation, one of America’s leading faith-based programs for combat trauma and resiliency. Through Mighty Oaks, Chad and his team have ministered to thousands of active-duty troops, veterans, and first responders, offering Christ-centered healing from the “unseen wounds” of war and service. In 2021, he led one of the largest civilian rescue operations in U.S. history during the Afghanistan withdrawal, helping evacuate more than 17,000 people from Taliban-controlled territory, proving that his zeal for life, liberty, and brotherhood is more than talk.
		 """,
		'image_y': -40,
	        "orgs": [{"icon": "shield", "name": "Mighty Oaks Foundation", "subtitle": "Healing combat trauma"}, {"icon": "star", "name": "Force Recon Marine", "subtitle": "JSOC Operator"},{"icon": "star", "name": "Author", "subtitle": ""}],
},
	{
		'name': 'Nate Schatzline',
		'image': 'img/speakers/NateS_5.1.png',
		'alt': 'Nate Schatzline',
		'titles': ['Preaching Pastor, Mercy Culture', 'Texas House of Representatives', 'Founder, For Liberty & Justice'],
		'bio': """
			Nate Schatzline is a Texas state representative, former pastor, and young warrior-legislator fighting to drag biblical conviction back into the public square. A devoted husband and father of two boys, he founded For Liberty & Justice to mobilize local churches toward reformation in government and has worked on the front lines against human trafficking through The Justice Reform in Fort Worth. Before his election to the Texas House, Nate spent nearly a decade pastoring and discipling the next generation, overseeing youth and young adult movements and mentoring hundreds of at-risk students with a clear message: you are not a victim of your environment; you were born on purpose, with purpose. Now serving in the legislature, he brings that same pastoral backbone to the fight for life, freedom, and the future of Texas families. His viral speeches on the House floor have made him a rising star in conservative politics, where he’s known for his fearless, no-notes style and his refusal to back down in the face of leftist attacks. His legislative accomplishments include protecting the unborn, defending parental rights, and pushing back against the woke agenda in schools and government, all while calling believers to get involved and fight for their values with courage and conviction.
		 """,
        'shrink': 0.85,
        'image_x': -20,
	        "orgs": [{"icon": "flag", "name": "Texas House of Reps", "subtitle": "Public servant"}, {"icon": "shield", "name": "For Liberty & Justice", "subtitle": "Founder"}, {"icon": "shield", "name": "Mercy Culture Church", "subtitle": "Preaching Pastor"}],
},
	{
		'name': 'Josh Howerton',
		'image': 'img/speakers/JoshH_2.png',
		'alt': 'Josh Howerton',
		'titles': ['Lead Pastor, Lakepointe Church', 'Live Free Podcast'],
		'bio': """
            Josh Howerton is the senior pastor of Lakepointe Church, a multi-site, multilingual movement with multiple campuses across the Dallas area committed to helping people know Christ, live free, and change the world. Before taking the helm at Lakepointe, Josh studied Biblical Studies at Union University, continued his theological training at Southern Seminary, and spent a decade pastoring The Bridge Church in Nashville. He hosts the “Live Free with Josh Howerton” podcast, where he speaks straight to men and women about sin, freedom, marriage, and mission in a style that connects with a broad audience. Under his leadership, Lakepointe has become a place where men are challenged to throw off religious apathy, embrace authentic repentance, and step into spiritual leadership at home, in church, and in culture.
            *Due to scheduling conflict, Josh will be joining us via video message.*
		 """,
		'shrink': 0.85,
        'image_x': 20,
	        "orgs": [{"icon": "church", "name": "Lakepointe Church", "subtitle": "Dallas multi-site"}, {"icon": "podcast", "name": "Live Free Podcast", "subtitle": ""}],
},

	{
		'name': 'Tim Barton',
		'image': 'img/speakers/TimB_2.png',
		'alt': 'Tim Barton',
		'titles': ['President of WallBuilders', 'The American Story Podcast'],
		'bio': """
			Tim Barton is the president of WallBuilders, a national pro-family organization dedicated to recovering America’s true history and biblical foundations. Standing in the legacy of his father, David Barton, Tim spends countless hours in original founding-era documents, unearthing the forgotten stories of statesmen, pastors, and patriots who built this nation on faith and moral courage. He is a sought-after speaker and media voice, appearing on outlets like Ben Shapiro, Glenn Beck, PragerU, and TBN to expose the lies in modern revisionist history and arm believers with the truth about our religious and constitutional heritage. An ordained minister, Tim has invested deeply in young people as a pastor, athletic director, and coach, helping forge a generation that loves God, loves America, and understands the duty that comes with both.
		 """,
	        "orgs": [{"icon": "book", "name": "WallBuilders", "subtitle": "Founding-era documents"}, {"icon": "podcast", "name": "American Story Podcast", "subtitle": "True American history"}],
},
	{
		'name': 'Dave Barton',
		'image': 'img/speakers/DaveB_2.png',
		'alt': 'Dave Barton',
		'titles': ['Founder of WallBuilders', 'The WallBuilders Show', 'Author & Historian'],
		'bio': """
			David Barton is the founder of WallBuilders and one of the most influential Christian voices on America’s religious and constitutional heritage. Drawing from one of the largest private collections of original founding-era documents in the country, he has written numerous bestselling books and served as a consultant to state and federal legislators, even participating in cases at the U.S. Supreme Court. David has helped shape history and social studies standards in states like Texas and California and has assisted in producing textbooks used in schools nationwide, pushing back against secular revisionism with hard primary-source evidence. For decades, he has been calling pastors, parents, and patriots alike to reject historical amnesia and embrace the call to rebuild our culture on the same biblical foundations that made this nation free.

			David Barton is the founder of WallBuilders and one of the most influential Christian voices on America’s religious and constitutional heritage. Drawing from one of the largest private collections of original founding-era documents in the country, he has written numerous bestselling books and served as a consultant to state and federal legislators, even participating in cases at the U.S. Supreme Court. David has helped shape history and social studies standards in states like Texas and California and has assisted in producing textbooks used in schools nationwide, pushing back against secular revisionism with hard primary-source evidence. For decades, he has been calling pastors, parents, and patriots alike to reject historical amnesia and embrace the call to rebuild our culture on the same biblical foundations that made this nation free.
		 """,
		'image_y': -20,
        'shrink': 0.92,
	        "orgs": [{"icon": "book", "name": "WallBuilders", "subtitle": "Founder & Historian"}, {"icon": "podcast", "name": "The WallBuilders Show", "subtitle": "Weekly radio"}],
},
	{
		'name': 'Adam James',
		'image': 'img/speakers/AdamJ_3.png',
		'alt': 'Adam James',
		'titles': ['Exec. Pastor, Grace City Church', 'Washington State Rep. Candidate'],
		'bio': """
			Adam James is one of the founding pastor of Grace City Church, where he is raising up men and families to follow Jesus with grit, joy, and conviction in everyday life. Drawing on years of local-church ministry, Adam preaches a clear, Christ-centered gospel and calls men out of passivity into sacrificial service at home, in church, and in the public square. In a cultural moment obsessed with image and comfort, he is steadily building a biblical, masculine culture that prizes repentance, responsibility, and brotherhood over hype. Adam is running for state representative in Washington, where he’s challenging the status quo with a bold vision for freedom, family, and faith in the public square. 

			If you send me a link to his church’s bio page, I can tighten this further to match his exact story and accomplishments.
		 """,
		'shrink': 0.9,
	        "orgs": [{"icon": "church", "name": "Grace City Church", "subtitle": "Local pastoral leadership"}, {"icon": "church", "name": "Running for state representative", "subtitle": "Washington State"}],
},
	{
		'name': 'Ryan Visconti',
		'image': 'img/speakers/RyanV_2.png',
		'alt': 'Ryan Visconti',
		'titles': ['U.S. Army Veteran', 'Lead Pastor, Generation Church'],
		'bio': """
			Ryan Visconti is the lead pastor of Generation Church in Arizona and a former U.S. Army captain who first sensed the call to ministry while deployed to Iraq. A graduate of Arizona State University with a master’s degree in theology from Liberty University, he brings a soldier’s clarity and a theologian’s depth to the pulpit. Under his leadership, Generation Church has grown tenfold in attendance, becoming a bold, Spirit-filled witness in one of the fastest-growing—and most contested—regions of the country. Ryan is known for preaching without compromise on cultural flashpoints while calling men to holiness, courage, and the kind of faith that can withstand fire.
		 """,
	        "orgs": [{"icon": "church", "name": "Generation Church", "subtitle": "Arizona"}, {"icon": "star", "name": "U.S. Army", "subtitle": "Former Army Captain"}],
},
	{
		'name': 'Nick Freitas',
		'image': 'img/speakers/NickF_2.png',
		'alt': 'Nick Freitas',
		'titles': ['Green Beret Combat Veteran', 'Virginia House of Delegates', 'Social Media Influencer'],
		'bio': """
			Nick Freitas is a Green Beret combat veteran, husband and father, and member of the Virginia House of Delegates who brings battlefield clarity to the chaos of modern politics. He served 11 years in the U.S. Army, including two combat tours in Iraq with Army Special Forces, before stepping into public office to fight for limited government, individual liberty, and the rights of families. As a delegate, Nick has served on key committees dealing with transportation, technology, courts, education, and finance, and he’s become widely known for viral, no-notes floor speeches dismantling leftist arguments with calm, surgical logic. His growing social media presence has turned him into a go-to example of what it looks like when a warrior-statesman brings courage, competence, and conviction into the halls of power.
		 """,
         'shrink': 0.9,
	        "orgs": [{"icon": "flag", "name": "Virginia House of Delegates", "subtitle": "Limited government"}, {"icon": "star", "name": "Green Beret", "subtitle": "11-year Army veteran"}],
},
]